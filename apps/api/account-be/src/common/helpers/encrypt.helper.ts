...
export const hashJwtToken = (token: string, salt: string): { hash: string } => {
  // 해시 함수 생성
  const hash = crypto
...
//   storedRefreshToken: string, // db 저장된 token
//   refreshToken: string, // 사용자로부터 받은 token
//   salt: string // JWT secret
// ): boolean => {
